...
const EditProjectModal: React.FC<EditProjectModalProps> = ({ isOpen, onClose, project }) => {
  const { updateProject } = useTicketStore();
          ...
          >
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <h3 style={{ fontSize: '1.25rem', fontWeight: 800, letterSpacing: '-0.02em' }}>Edit Project Workspace</h3>
              </div>
              <button 
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} style={{ padding: '1.75rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.25rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    <Code size={16} style={{ position: 'absolute', left: '1rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
                    <input 
                      type="text"
                      placeholder="SA"
                      maxLength={4}
                      value={formData.code}
                      className="input-field"
                      style={{ paddingLeft: '2.75rem', backgroundColor: 'rgba(255,255,255,0.02)', color: 'var(--text-muted)', cursor: 'not-allowed' }}
                      disabled={true}
                    />
                  </div>
                </div>
